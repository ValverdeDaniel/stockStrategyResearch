...
class FinancialTableGenerator:
    ...
    def fetch_forecast_data(self):
        ...
        # Also check for any fiscal year-end dates in 2025, 2026, 2027
        for period_key in earnings_trend.keys():
            # Check if this is a date key for years we want
            for target_year in [2025, 2026, 2027]:
                if period_key.startswith(str(target_year)) and target_year not in forecast_data:
                    trend = earnings_trend[period_key]
                    ...
                    # Add metadata
                    year_data["number_of_analysts"] = trend.get("earningsEstimateNumberOfAnalysts", 0)
                    year_data["period_key"] = period_key  # Store which period this came from

                    if year_data and (year_data.get("eps") or year_data.get("revenue_per_share")):
                        forecast_data[target_year] = year_data
                        print(f"Found {target_year} forecast data in {period_key}")
                        break  # Found data for this year, no need to check other dates
